    ...
    setBookid(bok);
      ...
      axios.post("http://localhost:3001/api/machineModel",{isbn:bok}).then((data)=>{
        ...
        data.data.map((book) => (       
          ...
          "&key=" +
          apiKey +
          "&maxResults=1").then((data)=>{
            newArray.push(data);
          })
        ));
        setRecommend(newArray);
        console.log(reco);
      ...
      >
       ...
       {result.map((book) => (
        ...
        >
          ...
          <Card>
          <CardBody>
                <h3 fontWeight="fontWeightBold">BOOK RECOMMENDATIONS BASED ON THIS SEARCH</h3> 

          </CardBody>
          </Card>
          {reco.map((boo) => (
          <GridItem xs={12} sm={6} md={6} key={boo.id}>
            <Card>
              <CardHeader color="success" stats icon>
                <CardIcon color="success">
                  <img
                    ...
                    width="120px"
                    src={
                     boo.data.items.[0].volumeInfo.imageLinks === undefined
                        ? "data:image/png;base64,"
                        : boo.data.items.[0].volumeInfo.imageLinks.smallThumbnail
                    }
                  />
                </CardIcon>
                <p className={classes.cardTitleBlack}> {boo.data.items.[0].volumeInfo.title}</p>
                <h6 className={classes.cardCategoryBlack}>
                  {" "}
                  {boo.data.items.[0].volumeInfo.description === undefined
                    ? "Unknown"
                    : boo.data.items.[0].volumeInfo.description}
                </h6>
              </CardHeader>
              <CardBody>
              ...
              <CardFooter stats>
                <div className={classes.stats}>
                  <DateRange />
                  {boo.data.items.[0].volumeInfo.authors === undefined
                    ? "Unknown"
                    : boo.data.items.[0].volumeInfo.authors}
                  {boo.data.items.[0].volumeInfo.authors === undefined
                    ? "Unknown"
                    : boo.data.items.[0].volumeInfo.averageRating}
                  {boo.data.items.[0].volumeInfo.categories === undefined
                    ? "Unknown"
                    : boo.data.items.[0].volumeInfo.categories}

                  {boo.data.items.[0].volumeInfo.publishedDate === undefined
                    ? "Unknown"
                    : boo.data.items.[0].volumeInfo.publishedDates}
                  {boo.data.items.[0].volumeInfo.pageCount === undefined
                    ? "Unknown"
                    : boo.data.items.[0].volumeInfo.pageCount}
                  {boo.data.items.[0].volumeInfo.subtitle === undefined
                    ? "Unknown"
                    : boo.data.items.[0].volumeInfo.subtitle}
                </div>
              </CardFooter>
            </Card>
          </GridItem>
